...
public class Ejercicios2 {
	...
	public ArrayList<Movimiento> examenCargaMovimientos(String rutaFichero){
		try {
			...
			while ((registro=fichero.readLine() ) != null) {
				...
				movimiento = new Movimiento(campos[0], Integer.parseInt(campos[1]), Float.parseFloat(campos[2]), campos[3].charAt(0));
				listaMovimiento.add(movimiento);

			}
			System.out.println(listaMovimiento);
			fichero.close();
			System.out.println("Creada la lista de movimientos..");
	...
	// ###### EJERCICIO 2
	 ...
	 public float examenCalculaSaldo(float[][] movimientos, float saldoInicial) {
		 ...
		 for (int i = 0; i < movimientos.length; i++) {
			for (int j = 0; j < movimientos[i].length; j++) { // tamaño fila
				saldoInicial = saldoInicial + (movimientos[i][j]);
				saldoFinal = saldoInicial;
			}
			
		}
		 System.out.printf("Saldo final: %.2f €\n", saldoFinal);
		return saldoFinal; // saldo final
	...
	//##### EJERCICIO 3
		 ...
		 public void devolverResumenVisitaMensual(String ficheroTurist) throws IOException{
			 // Abrir fichero lectura
			
			float matriz[][] = new float[7][12];
			String[] meses = { "ENERO", "FEBRERO", "MARZO", "ABRIL", " MAYO", "JUNIO", "JULIO", "AGOSTO", "SEPTIEMBRE",
					"OCTUBRE", "NOVIEMBRE", "DICIEMBRE" };
			int isla = 0;
			
			BufferedReader fichero = new BufferedReader(new FileReader(ficheroTurist));
			String registro;
			while((registro = fichero.readLine()) != null) {
				String [] campos = registro.split("&");
				
				// Recorrer la matriz por filas --> pasarlo datos a matriz
				for (int i = 0; i < campos.length;i++) {
						matriz[isla][i] =  Float.parseFloat(campos[i]);  //String a Float
				}
				isla++;
			 }
			
			//Recorrer matriz por columnas y mostrar la suma con el mes
			float [] resumenMes = new float[12];
			for (int j = 0; j < resumenMes.length; j++) {
				for (int i = 0; i < 7; i++) {
					 resumenMes[j]+= matriz[i][j];
				}
			}
			
		
			System.out.println("RESUMEN DE VISITANTES A CANARIAS\n");
			for (int i = 0; i < resumenMes.length; i++) {
				System.out.println(meses[i] +  " : \t" + resumenMes[i]*100);
			}
...
// Codificar AQUI los tres métodos del examen del 30 de marzo de 2017
